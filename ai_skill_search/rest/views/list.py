...
def get_jobadder_candidate_status_list(access_token, base_url):
    url = f"{base_url}candidates/lists/status"
...
def jobadder_candidate_status_list(request):
    user = request.user
    ...
    response = get_jobadder_candidate_status_list(
        platform.access_token, platform.base_url
    )
    if response.status_code == 401:
        try:
            new_token = platform.refresh_access_token()
            response = get_jobadder_candidate_status_list(new_token, platform.base_url)
