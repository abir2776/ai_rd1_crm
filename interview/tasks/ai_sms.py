...
def generate_ai_sms_instructions(
    job_title: str,
    job_id: int,
    application_id: int,
    candidate_name: str,
    ...
    """Generate AI instructions for SMS-based interview"""
    instructions = f"""You are an AI recruiter for Recruitment Direct conducting a text-based job interview via SMS. Keep ALL responses under 150 characters.
...
## Critical Rules
- MAXIMUM 150 characters per message (strict limit)
- Ask ONE question at a time
...
[END_INTERVIEW:unsuccessful] - If candidate:
  ...
  * Message: "Thanks for your time. Unfortunately, you don't meet requirements. Feel free to apply for other positions. Goodbye."
...
- Keep final message under 150 characters including the tag
"""
...
def get_ai_response(
    ...
    """
    Get AI response using GPT-4o model.
...
def initiate_sms_interview(
    ...
    try:
        ...
        if sms_sent:
            ...
            update_application_status_after_sms(organization_id, application_id, "sent")
        else:
            print(f"Failed to send initial SMS to {to_number}")

    except Exception as e:
        print(f"Error initiating SMS interview: {str(e)}")
...
def process_candidate_sms_response(candidate_phone: str, candidate_message: str):
    ...
    try:
        ...
        # Get AI response
        ai_message, interview_status = get_ai_response(
...
def update_application_status_after_sms(
    organization_id: int, application_id: int, interview_result: str
):
    """Update application status based on SMS interview result"""
    try:
        config = AIMessageConfig.objects.get(organization_id=organization_id)
        ...
        print(f"No SMS config found for organization {organization_id}")
    except Exception as e:
...
@shared_task
def fetch_sms_candidates(config):
    """Fetch candidates eligible for SMS interview"""
    access_token = config.platform.access_token
    ...
    try:
        ...
        print(f"Total SMS candidates: {len(candidates)}")
        return candidates

    except Exception as e:
        print(f"Error fetching SMS candidates: {str(e)}")
        return []


@shared_task
def bulk_sms_interviews(organization_id: int = None):
    """Initiate bulk SMS interviews for an organization"""
    try:
        config = AIMessageConfig.objects.get(organization_id=organization_id)
    except AIMessageConfig.DoesNotExist:
        print(f"No SMS config found for organization {organization_id}")
    ...
    candidates = fetch_sms_candidates(config)

    if not candidates:
        print("No candidates to interview via SMS")
        return
    ...
    print(f"Scheduled {len(candidates)} SMS interviews")


@shared_task
def initiate_all_sms_interviews():
    """Initiate SMS interviews for all subscribed organizations"""
    ...
    for organization_id in subscribed_organization_ids:
        print(f"Initiating SMS interviews for organization {organization_id}")
        bulk_sms_interviews.delay(organization_id)
